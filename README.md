...
投稿は、検出されたイベントの平均精度、タイムスタンプ誤差許容閾値の平均、イベントクラスの平均で評価されます。
...
- **割り当て**
  - 予測されたイベントは、グラウンド・トゥルース・イベントとマッチングされます。
- **スコアリング**
  - 予測の各グループは、平均精度を介して、対応する地上真実イベントのグループに対してスコアリングされます。
- **削減**
  - 複数のAPスコアが平均化され、1つの総合スコアが生成されます。

##### 割り当て

同じイベント x 許容範囲 x series_id グループ内の予測およびグランドの真実の各セットについて、各グランドの真実を、許容される許容範囲内で発生する最も信頼性の高い一致しない予測にマッチさせる。

いくつかの地上真理は予測にマッチしないかもしれないし、いくつかの予測は地上真理にマッチしないかもしれない。しかし、それらはまだスコアリングで考慮されます。

##### スコアリング

各 series_id 内のイベントを収集し、各イベント x トレランス・グループの平均精度スコアを計算します。平均精度スコアは、予測値の信頼スコアしきい値を減少させることで生成される精度-再現曲線の下の面積です。この計算では、しきい値を超えてマッチした予測は TP、マッチしていない予測は FP としてスコア付けされる。一致しない地上真理はFNとしてスコア付けされる。

##### 削減

最終的なスコアは、上記のAPスコアの平均値である。

### Submit

series_idで示される各シリーズについて、そのシリーズで発生する各イベントを、イベントのタイプとイベントが発生したステップ、およびそのイベントの信頼度スコアを与えて予測する。評価指標はさらに、イベントの列挙を持つrow_idを必要とする。
...
グランド・トゥルースのアノテーションは、（データ・ページに記載されているように）一定の規則に従って作成されていますが、あなたの投稿ファイルにはそのような制限はないことに注意してください。
...
- **2023年9月5日** - 開始日
- **2023年11月28日** - エントリー締切日。出場するには、この日までに競技規則に同意する必要があります。
- **2023年11月28日** - チーム合併締切日。この日が、参加者がチームに加入または合併できる最終日となる。
- **2023年12月5日** - 最終提出期限。
